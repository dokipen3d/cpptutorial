...
//add ai player
//introduce new part of loop to start adding guesses
//add blank grid to track enemy positions
//add attemptHit(). simple bool makes dealing with logic at the game level easy. the status switching is done at the item level
//add a addToTracking function.
//add a way to check the rest of ship
//dont forget to initialize size of tracking grid
//main problem now is that we have slicing of the vector of items
...
public:
    ...
    bool AttemptHit(int posRow, int posCol) {
        ...
        BattleShip::SeaItem& item = grid[Convert2dTo1D(posRow, posCol)];
    ...
    void addHitPositionToTrackingGrid(int posRow, int posCol, BattleShip::eShipStatus hitormiss ) {
        ...
        enemyTrackingGrid[Convert2dTo1D(posRow, posCol)] = hitormiss;
...
private:
    ...
    vector<BattleShip::SeaItem> enemyTrackingGrid;
    ...
    void startGameLoop() {
        while ((eStatus == GameStatus::MENU) || (eStatus == GameStatus::PLAYING))
        {
            ...
            if(AI.AttemptHit(a,b)) {
                me.addHitPositionToTrackingGrid(a, b, BattleShip::eShipStatus::HIT);
            }
            else {
                me.addHitPositionToTrackingGrid(a, b, BattleShip::eShipStatus::MISS);
            }
